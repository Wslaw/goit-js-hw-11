...
// 3.Створити функцію для рендеринга карток зображень
// 4.Створити функцію для розмітки карток зображень
// 5.Створити скролл
...
const lightbox = new (0, _simplelightboxDefault.default)(".gallery-item");
let searchQueryRes = "";
let currentPage = 1;
// let backButton;
...
form.addEventListener("submit", async (event)=>{
    ...
    if (!searchQueryRes) {
        (0, _notiflixDefault.default).Notify.failure("Please enter a search query");
        return;
    }
    currentPage = 1;
    gallery.innerHTML = "";
...
const fetchImages = async ()=>{
    try {
        const response = await (0, _axiosDefault.default).get(`${BASE_URL}?key=${API_KEY}&q=${searchQueryRes}&image_type=photo&orientation=horizontal&safesearch=true&page=${currentPage}&per_page=40`);
        const { totalHits , hits  } = response.data;
        // console.log('currentPage:', currentPage);
        // console.log('hits:', hits);
        console.log("totalHits:", totalHits);
        if (hits.length === 0) {
            (0, _notiflixDefault.default).Notify.warning("Sorry, there are no images matching your search query. Please try again.", 5000);
            return;
        }
        // Render
        renderImages(hits);
        // LoadMore- is hidden?
        if (currentPage * 40 <= totalHits) loadMoreBtn.classList.remove("is-hidden");
        else {
            loadMoreBtn.classList.add("is-hidden");
...
const renderImages = (images)=>{
    /*Перебираємо масив зображень та створюємо для нього карточку:*/ const imageCards = images.map((image)=>{
        /*Викликаємо фукцію createImageCard для створення HTML розмітки:*/ const cardMarkUp = createImageCard(image);
        /* возвращаем разметку карточки: */ return cardMarkUp;
...
},{"simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","axios":"jo6P5","notiflix":"5z0Oc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"9ydBq":[function(require,module,exports) {
/*!
